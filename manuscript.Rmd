...
Impact measures whether the scholarly effort and the implementation and dissemination actually had an effect on the field.  
...
To assess this, we propose to simply provide a checkbox, in which the author indicates whether a scientific software package is actively maintained (e.g., software has a regular release cycle or an update in the last six months).
...
However, citations suffer from serious shortcomings when used to evaluate the impact of research software, particularly, when used to compare candidates in the context of hire and promotion.
...
Further, functionality of successful, modular scientific software is ideally imported from other software packages to avoid duplication of code and enable faster development of new software.
...
We object to the use of Github stars as a general metric.
First of all, it is tied only to a single, commercial platform.
More importantly, we question the validity of such metrics that, like rating systems on commercial platforms, are too easy to be gamed.
...
However, we think an open license is central for assessing both rigor and (potentially) increasing impact.
...
Therefore, we propose to penalize software if it does not adhere to an open-source licenses (those approved in a review process by the Open Source Society (https://opensource.org/licenses) by allowing them only half of the maximum achievable total points. <!--Alternatively, one may indirectly score openness by assessing the indicators of rigor as discussed in the second part of this commentary, where it is only possible to gain points for certain aspects of rigor if the software is open-source.-->
...
By also incorporating the size of the user base, it confounds *usablity* (as an aspect of rigor) with *usage* (as an aspect of impact).
...
We instead propose to assess impact independently of rigor in phase I.
In the following, we propose ways to assess the different aspects of rigor separately, and more clearly.
...
We propose to use these as equally weighted indicators in lieu of the proposed coarse reusability indicator (item 17 in Table 3 of the proposal).
...
Tests are essential to discover invalid or incorrect functionality, investigate scalability of code and reveal poor design choices.
There are a variety of useful tests, such as unit tests of software subcomponents or tests of software functionality at larger scale (e.g., see the testthat package in R).
...
However, we believe that we should give a point for software that promises that major functionality is covered by tests.
...
For example, there are tutorials showcasing the usage of software with examples, and there is application programming interface (API) documentation for individual functions and classes.
...
We propose to score two aspects of maintenance separately, such as active bug fixing, documenting changes in logs, and the possibility to report bugs as a user (for example via tickets or mailing lists).

## Measuring Impact

In our opinion, total citation metrics, number of users, downloads per month and similar may provide a coarse measure for the impact of a software package, even though it is important to note the shortcomings we described above.
...
We therefore argue that qualitative merit statements are much more useful to assess impact of research software and that this should be the primary statement for committee members to evaluate if they are less concerned with the technical aspects of research software development.
...
We believe that the current proposal should aim to better reflect the distinction between rigor and impact.
